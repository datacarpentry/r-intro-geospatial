...
We can explore the data in terms of content and structure using various functions.

* Size:
    * `dim(nordic)` - returns the dimensions of the object
    * `nrow(nordic)` - returns the number of rows
    * `ncol(nordic)` - returns the number of columns

* Content:
    * `head(nordic)` - shows the first 6 rows
    * `tail(nordic)` - shows the last 6 rows

* Names:
    * `names(nordic)` - returns the column names 
    * `rownames(nordic)` - returns the row names

* Summary:
    * `summary(nordic)` - summary statistics for each column
    * `str(nordic)` - structure of the object and information about the class, length and content of  each column
...
data structures are, and how they behave. 
 Data structures in R include vectors, matrices, dataframes and lists. We incude an example below to have a glimpse on various structures.

Vectors are the basic data structure in R. The data type in the vector has to be in the same data types, i.e. numeric, characters, factors or logical.
...
**Matrices** are similar to vectors, but in two-dimensions.
...
**Dataframes** are similar to matrices (two-dimensional), but it can contain multiple data types.
```{r}
DF = data.frame(
  country = c('Denmark', 'Sweden', 'Norway'),
  year = c(2002, 2002, 2002),
  lifeExp = c(77.2, 80, 79)
)
DF
```

**Lists** are similar to vectors, in which each elements in the lists can contain any data structures
```{r}
L=list(v,M,DF)
L
```
